...
def read_file(file_suffix):
    ...
    cols_to_remove = [col for col in df.columns if col.startswith("ism") or col.startswith("gradxinp") or col.startswith("isa")]
...
for dataset in ["promoters_hepg2","enhancers_hepg2","promoters_k562","enhancers_k562"]:
    for track_num in get_track_num(dataset):
        for feat_imp in ["gradxinp","ism","isa"]:
            logger.info(f"Processing {dataset}, track {track_num}")
            df=df_orig[df_orig["dataset"]==dataset].reset_index(drop=True)
            df=df[df[f"max_{feat_imp}_{track_num}"]>0].reset_index(drop=True)
            df=bin_and_label(df,f"max_{feat_imp}_{track_num}",[0, 0.05, 0.1, 0.2, 0.5, np.inf])
            # for common variants
            df_or,n_list=calc_or_by_various_thresholds(df,"max_af",threshold_list,"larger",f"max_{feat_imp}_{track_num}_bin")
            color_mapping = dict(zip(threshold_list, colors_list))
            n_mapping = dict(zip(threshold_list, n_list))
            plot_or_af(df_or,color_mapping,n_mapping,"larger",f"max({feat_imp}){dataset}, track {track_num})",f"Plots_af/{feat_imp}_{dataset}_{track_num}.pdf")
...
for dataset in ["promoters_hepg2","enhancers_hepg2","promoters_k562","enhancers_k562"]:
    for track_num in get_track_num(dataset):
        for feat_imp in ["gradxinp","ism","isa"]:
            ...
            df=bin_and_label(df,f"max_{feat_imp}_{track_num}",[0, 0.05, 0.1, 0.2, 0.5, np.inf])
            # for common variants
            df_or,n_list=calc_or_by_various_thresholds(df,"max_241way",threshold_list,"larger",f"max_{feat_imp}_{track_num}_bin")
            color_mapping = dict(zip(threshold_list, colors_list))
            n_mapping = dict(zip(threshold_list, n_list))
            plot_or_phylop(df_or,color_mapping,n_mapping,"larger",f"max({feat_imp}){dataset}, track {track_num})",f"Plots_phylop/{feat_imp}_{dataset}_{track_num}.pdf")