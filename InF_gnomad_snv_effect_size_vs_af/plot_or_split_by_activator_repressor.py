...
df_promoters_hepg2.columns=["chrom","start","end","name","REF","ALT","AF",'Name','Score','Strand']+["track_"+str(i) for i in range(16)]


def filter_by_activator_or_repressor(df,dataset,track_num,mode):
    # read filter
    df_filter=pd.read_csv(f"/isdata/alab/people/pcr980/DeepCompare/Pd5_motif_info/motif_info_thresh_500_{dataset}.csv")
    df_filter=df_filter.iloc[:,2:]
    if mode=="activator":
        df_filter=df_filter[df_filter[f"isa_track{track_num}"]>0].reset_index(drop=True)
    elif mode=="repressor":
        df_filter=df_filter[df_filter[f"isa_track{track_num}"]<0].reset_index(drop=True)
    else:
        raise ValueError("mode must be either activator or repressor")
    # convert to BedTool
    bed_df=BedTool.from_dataframe(df)
    bed_filter=BedTool.from_dataframe(df_filter)
    bed_df_filtered=bed_df.intersect(bed_filter)
    df_res=bed_df_filtered.to_dataframe(names=["chrom","start","end","ID","REF","ALT","AF",'Name','Score','Strand']+["track_"+str(i) for i in range(16)])
...
for dataset in ["promoters_hepg2","promoters_k562","enhancers_hepg2","enhancers_k562"]:
    for track_num in get_track_num(dataset):
        for mode in ["activator","repressor"]:
            logger.info(f"Processing {dataset}, track {track_num}",mode)
            df=df_orig[df_orig["dataset"]==dataset].reset_index(drop=True)
            # add activator/repressor filter
            df=filter_by_activator_or_repressor(df,dataset,track_num,mode)
            ...
            plot_or(df_or,color_mapping,n_mapping,f"Odds ratio ({dataset}, track {track_num})",f"Plots_or/or_{dataset}_track{track_num}_{mode}.pdf")


# nohup python3 plot_or_split_by_activator_repressor.py > plot_or_split_by_activator_repressor.out &
