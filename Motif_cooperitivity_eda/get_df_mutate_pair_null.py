...
def extract_motif_and_null_info(jaspar_annotator,remap_annotator,region):
    ...
    df_motif=jaspar_annotator.annotate(region,by="contained")
...
def get_isms(df):
    for i in range(8):
        df[f"ism1_null_track{i}"]=df[f"pred_orig_track{i}"]-df[f"pred_mut1_null_track{i}"]
        df[f"ism2_track{i}"]=df[f"pred_orig_track{i}"]-df[f"pred_mut2_track{i}"]
        df[f"ism_both_null_track{i}"]=df[f"pred_orig_track{i}"]-df[f"pred_mut_both_null_track{i}"]
    return df
...
def write_mutation_res_for_one_region(seq_extractor,
    ...
    df_mutation=get_isms(df_mutation)
