setwd("/maps/projects/ralab/people/pcr980/DeepCompare/Curate_motif_annot/")
...
name_mapper <- read.csv("/maps/projects/ralab/people/pcr980/Resource_HTF/DatabaseExtract_v_1.01.csv",row.names = 1)
...
remap_hepg2 <- import.bed("/maps/projects/ralab/people/pcr980/Resource_ReMap2022/ReMap2022_Hep-G2.bed")
remap_k562 <- import.bed("/maps/projects/ralab/people/pcr980/Resource_ReMap2022/ReMap2022_K-562.bed")
...
#-------------------------------------------------------
tf_expressed_hepg2 <- tf_expr_hepg2 %>% filter(TPM>0) %>% pull(HGNC.symbol) 
tf_expressed_hepg2 <- unique(c(tf_expressed_hepg2,tf_hepg2))

tf_expressed_k562 <- tf_expr_k562 %>% filter(TPM>0) %>% pull(HGNC.symbol) 
