...
source("/maps/projects/ralab/people/pcr980/DeepCompare/SNP_interpretation_MPRA/Rcode/function_utils.R")
    # correlation_avg_fr_by_element
    # calculate_enformer_delta
...
cor_dc_reg <- subset(cor_dc_reg, !((Element %in% c('F9', 'LDLR', 'LDLR.2','SORT1',"SORT1-flip","SORT1.2")) & grepl("k562", Method)) & 
                         !((Element %in% c('BCL11A','GP1BA', 'HBB', 'HBG1', 'PKLR-24h', 'PKLR-48h')) & grepl("hepg2", Method)))
cor_dc_reg$Model <- "DeepCompare_regression"
...
correlations <- rbind(cor_dc_class,cor_dc_reg,cor_enformer)
...
correlations$Model <- gsub("DeepCompare_", "DeepCompare\n", correlations$Model)
