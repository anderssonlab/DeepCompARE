...
df=pd.read_csv("/isdata/alab/people/pcr980/DeepCompare/Pd8_TF_cooperativity/tf_pair_cooperativity_index_post_filter.csv")
...
# sns kde plot
sns.kdeplot(data=df,x="cooperativity_index",hue="same_family",common_norm=False)
plt.savefig("Plots/cooperativity_index_same_family.pdf")
plt.close()

# calculate p value
df_same_family=df[df["same_family"]==True]
df_diff_family=df[df["same_family"]==False]
mannwhitneyu(df_same_family["cooperativity_index"],df_diff_family["cooperativity_index"])
df_same_family["cooperativity_index"].median()
df_diff_family["cooperativity_index"].median()